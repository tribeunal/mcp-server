...
was spent. A careful agent will not: `tribeunal_get_jury_duty_status`'s `allowance`
block reports a DIFFERENT daily counter (jury-duty sessions, `dailyMax: 27`) and
shows plenty remaining, so the transcript that checked its allowance correctly
refused to endorse the message's claim and was scored as a failure for being right.
